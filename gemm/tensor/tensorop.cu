...
__global__ void myHGEMMAlignedV1(
    ...
    wmma::fragment<wmma::matrix_a, 16, 16, 16, half, wmma::row_major> frag_a[4][2]; // 64 x 16 x 2
    wmma::fragment<wmma::matrix_b, 16, 16, 16, half, wmma::row_major> frag_b[2][4]; // 16 x 64 x 2
    wmma::fragment<wmma::accumulator, 16, 16, 16, half> frag_c[4][4];               // 64 x 64
    ...
    int comp_c_frag_m = wid >> 2;
    int comp_c_frag_n = wid & 3;
    ...
    for (int bk = 0; bk < K / BK; bk++) {
        ...
        wmma::load_matrix_sync(frag_a[1][0], &s_a[comp_c_frag_m * 64 + 16][ 0], BK + APAD);
        wmma::load_matrix_sync(frag_a[2][0], &s_a[comp_c_frag_m * 64 + 32][ 0], BK + APAD);
        wmma::load_matrix_sync(frag_a[3][0], &s_a[comp_c_frag_m * 64 + 48][ 0], BK + APAD);
        wmma::load_matrix_sync(frag_a[0][1], &s_a[comp_c_frag_m * 64     ][16], BK + APAD);
        wmma::load_matrix_sync(frag_a[1][1], &s_a[comp_c_frag_m * 64 + 16][16], BK + APAD);
        wmma::load_matrix_sync(frag_a[2][1], &s_a[comp_c_frag_m * 64 + 32][16], BK + APAD);
        wmma::load_matrix_sync(frag_a[3][1], &s_a[comp_c_frag_m * 64 + 48][16], BK + APAD);
        ...
        for (int i = 0; i < 4; i++) {
            ...
            for (int j = 0; j < 4; j++) {
                ...
                wmma::mma_sync(frag_c[i][j], frag_a[i][1], frag_b[1][j], frag_c[i][j]);
            }
        }
...
__global__ void myHGEMMAlignedV2(
    ...
    size_t s_a_base_addr = __cvta_generic_to_shared(s_a[0]);
    size_t s_b_base_addr = __cvta_generic_to_shared(s_b[0]);
    size_t load_a_smem_addr_0 = s_a_base_addr + OFFSET(load_a_smem_m, load_a_smem_k, BK + APAD) * sizeof(half);
    size_t load_a_smem_addr_1 = load_a_smem_addr_0 + (BK + APAD) * sizeof(half);
    size_t load_b_smem_addr_0 = s_b_base_addr + OFFSET(load_b_smem_k, load_b_smem_n, BN + BPAD) * sizeof(half);
    size_t load_b_smem_addr_1 = load_b_smem_addr_0 +     (BN + BPAD) * sizeof(half);
    size_t load_b_smem_addr_2 = load_b_smem_addr_0 + 2 * (BN + BPAD) * sizeof(half);
    size_t load_b_smem_addr_3 = load_b_smem_addr_0 + 3 * (BN + BPAD) * sizeof(half);
    ...
    for (int bk = 0; bk < K / BK; bk++) {
        ...
        // cooperative_groups::memcpy_async(tb, &s_a[load_a_smem_m    ][load_a_smem_k], 4, &a[load_a_gmem_addr        ], 4);
        // cooperative_groups::memcpy_async(tb, &s_a[load_a_smem_m + 1][load_a_smem_k], 4, &a[load_a_gmem_addr +     K], 4);
        // cooperative_groups::memcpy_async(tb, &s_b[load_b_smem_k    ][load_b_smem_n], 4, &b[load_b_gmem_addr        ], 4);
        // cooperative_groups::memcpy_async(tb, &s_b[load_b_smem_k + 1][load_b_smem_n], 4, &b[load_b_gmem_addr +     N], 4);
        // cooperative_groups::memcpy_async(tb, &s_b[load_b_smem_k + 2][load_b_smem_n], 4, &b[load_b_gmem_addr + 2 * N], 4);
        // cooperative_groups::memcpy_async(tb, &s_b[load_b_smem_k + 3][load_b_smem_n], 4, &b[load_b_gmem_addr + 3 * N], 4);
        // cooperative_groups::wait(tb);

        asm ("cp.async.ca.shared.global [%0], [%1], 16;\n" :
            : "l"(load_a_smem_addr_0), "l"(&a[load_a_gmem_addr        ]));
        asm ("cp.async.ca.shared.global [%0], [%1], 16;\n" :
            : "l"(load_a_smem_addr_1), "l"(&a[load_a_gmem_addr +     K]));
        asm ("cp.async.ca.shared.global [%0], [%1], 16;\n" :
            : "l"(load_b_smem_addr_0), "l"(&b[load_b_gmem_addr        ]));
        asm ("cp.async.ca.shared.global [%0], [%1], 16;\n" :
            : "l"(load_b_smem_addr_1), "l"(&b[load_b_gmem_addr +     N]));
        asm ("cp.async.ca.shared.global [%0], [%1], 16;\n" :
            : "l"(load_b_smem_addr_2), "l"(&b[load_b_gmem_addr + 2 * N]));
        asm ("cp.async.ca.shared.global [%0], [%1], 16;\n" :
            : "l"(load_b_smem_addr_3), "l"(&b[load_b_gmem_addr + 3 * N]));
...
int main() {
    ...
    */
    ...
    {
        printf("\nalgo = HGEMMDirectGlobalToShared\n");
